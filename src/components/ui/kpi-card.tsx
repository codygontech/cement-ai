...
export function KPICard({ 
  ...
  return (
    <Card className={cn("h-full hover:shadow-xl transition-all relative overflow-visible", className)}>
      ...
      <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-3 pt-0 px-6 relative z-10">
      ...
      <CardContent className="flex flex-col px-6 pt-0 pb-0 relative z-10 overflow-visible">
        <div className="flex items-baseline gap-2 mb-6 overflow-visible">
          <div className="text-4xl font-black metric-value" style={{ lineHeight: '1.4' }}>
            {value}
          </div>
        ...
        {change && (
          <div className={cn(
            "flex items-center gap-1.5 text-xs font-bold px-3 py-2.5 rounded-full w-fit shadow-md mb-4 overflow-visible",
        ...
        {children && <div className="mt-4 flex-1 overflow-visible">{children}</div>}
      </CardContent>
    </Card>
  );
}
